...
def train(
    model_id: str = typer.Option(
        "models/sft_warmup_ab", help="Path to SFT Audio model (Policy)."
    ),
    ref_model_id: str = typer.Option(
        "Qwen/Qwen2-7B", help="Path to Expert Text model (Reference)."
    ),
    json_path: Path = typer.Option(
        Path("data/processed/train_dpo_abtest_10k.json"), help="DPO A/B dataset."
    ),
    ...
    wandb_project: Optional[str] = typer.Option(
        "qwen2-audio-alld-ab", help="W&B project."
    ),
    wandb_run_name: Optional[str] = typer.Option(
        "alld-ab-finetune", help="W&B run name."
    ),
):
