"""
supervised-finetune-ab.py — A/B preference SFT training script for Qwen2-Audio.

Imports SFTDatasetAB and Qwen2AudioCollatorAB from data.py.
Each training sample contains two audio files and a preference response.
...
from asa.data import Qwen2AudioCollatorAB, SFTDatasetAB
...
def train(
    ...
    json_path: Path = typer.Option(
        Path("data/processed/train_nisqa_abtest_llama_10k.json"),
    ...
    wandb_project: Optional[str] = typer.Option(
        "qwen2-audio-sft-ab", help="Weights & Biases project name (None to disable)."
    ),
    wandb_run_name: Optional[str] = typer.Option(
        None, help="Weights & Biases run name."
    ),
):
    """Run A/B preference supervised fine-tuning on Qwen2-Audio."""
