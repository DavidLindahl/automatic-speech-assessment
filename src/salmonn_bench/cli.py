...
def run_mos(
    dataset_paths: list[Path] = typer.Option(
        ..., "--dataset-path", help="One or more MOS dataset JSONL paths."
    ),
    config_path: Path = typer.Option(
        Path("configs/salmonn_zeroshot.yaml"),
        "--config-path",
    ...
    data_root: Path = typer.Option(
        Path("data"),
        "--data-root",
        help="Local data root used to resolve dataset audio paths.",
    ),
    output_dir: Path = typer.Option(
        Path("results/salmonn_zeroshot"),
        "--output-dir",
        help="Directory for run outputs.",
    ),
    run_id: str | None = typer.Option(None, "--run-id", help="Optional run id label."),
    max_samples: int | None = typer.Option(
        None, "--max-samples", help="Optional cap per dataset for smoke runs."
    ),
) -> None:
...
def run_ab(
    dataset_paths: list[Path] = typer.Option(
        ..., "--dataset-path", help="One or more A/B dataset JSONL paths."
    ),
    config_path: Path = typer.Option(
        Path("configs/salmonn_zeroshot.yaml"),
        "--config-path",
    ...
    for dataset_path in dataset_paths:
        ...
        typer.echo(
            f"  {dataset_name}: Acc={metrics['accuracy']:.4f}, BLEU={metrics['bleu']:.4f}"
        )


@app.command("run-temporal")
def run_temporal(
    dataset_paths: list[Path] = typer.Option(
        ..., "--dataset-path", help="One or more temporal dataset JSON/JSONL paths."
    ),
    config_path: Path = typer.Option(
        Path("configs/salmonn_zeroshot.yaml"),
        "--config-path",
    ...
    """Run zero-shot SALMONN temporal localization benchmark."""
    config = load_benchmark_config(config_path)
    ...
    for dataset_path in dataset_paths:
        typer.echo(f"Running Temporal dataset: {dataset_path}")
        records = load_records(dataset_path)
        if max_samples is not None:
            records = records[:max_samples]
        ...
        prediction_rows: list[dict[str, Any]] = []
        for idx, record in enumerate(records, start=1):
            audio_raw = str(record["audio"])
            audio_path = resolve_audio_path(audio_raw, data_root)
            if not audio_path.exists():
                raise FileNotFoundError(f"Missing audio: {audio_path}")

            task_prompt = record.get(
                "question",
                "Please describe and evaluate the synthetic speech, and find timestamps for the degradation.",
            )
            pred = runner.infer_mos(audio_path, task_prompt)

            out_row = dict(record)
            out_row["resolved_audio"] = str(audio_path)
            # The scoring script looks for "predicted_response" among other fields
            out_row["predicted_response"] = pred
            prediction_rows.append(out_row)
        ...
        dataset_name = dataset_path.stem
        # Ensure we write out exactly what we were told in output_dir (we can just dump it directly)
        predictions_path = run_dir / f"{dataset_name}_predictions.json"

        # In temporal scoring, the evaluation is handled by scripts/eval/external_temporal.py
        # so we just need to dump the predictions for it to score later.
        dump_predictions(predictions_path, prediction_rows)
        typer.echo(f"  Wrote predictions for {dataset_name} to {predictions_path}")
