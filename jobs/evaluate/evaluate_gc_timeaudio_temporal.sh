#!/bin/bash
### ============================================================
### DTU HPC — TEMPORAL EVAL (greedy): global-caption TIMEAUDIO SFT model
### Evaluates the Setup-2 model (28615749, TimeAudio method: anchor/offset
### <a><f> tokens + learnable absolute-time embedding) on the filtered
### FOR/LIVE/P501 temporal test sets in the ANCHOROFFSET <a><f> format
### (MOS<=3, global-caption targets). Temporal IoU + start/end error, greedy.
###
...
#BSUB -q gpuh100
#BSUB -J eval-gc-timeaudio-temporal
#BSUB -n 4
...
#BSUB -M 48GB
#BSUB -W 4:00
#BSUB -o logs/eval_gc_timeaudio_temporal_%J.out
#BSUB -e logs/eval_gc_timeaudio_temporal_%J.err
...
MODEL_PATH="$PROJECT_DIR/models/sft_temporal_gc_timeaudio_h100"
OUTPUT_DIR="$EXPERIMENT_DIR/results/evaluation/temporal/gc_timeaudio_greedy"
