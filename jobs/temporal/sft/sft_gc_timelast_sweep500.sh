...
#BSUB -M 48GB
#BSUB -W 24:00
#BSUB -o logs/sft_gc_sweep500_%J.out
#BSUB -e logs/sft_gc_sweep500_%J.err
...
torchrun --nproc_per_node=1 scripts/train/supervised-finetune.py \
    ...
    --json-path data/processed/temporal/train_gc_timelast_sweep_500.json \
    --data-root data \
    --model-name "$EXPERIMENT_DIR/models/sft_gc_timelast_sweep500" \
    --bf16 \
    ...
    --install-time-tokens \
    --wandb-run-name "sft-gc-timelast-sweep500"
