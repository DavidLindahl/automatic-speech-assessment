...
#BSUB -M 48GB
#BSUB -W 24:00
#BSUB -o logs/sft_expc_detect_%J.out
#BSUB -e logs/sft_expc_detect_%J.err
...
torchrun --nproc_per_node=1 scripts/train/supervised-finetune.py \
    ...
    --json-path data/processed/temporal/train_nisqa_temporal_expc_detect_blend.json \
    --data-root data \
    --model-name "$EXPERIMENT_DIR/models/sft_expc_detect" \
    --bf16 \
    ...
    --install-time-tokens \
    --wandb-run-name "sft-expc-detect"
