#!/bin/bash
### ============================================================
### DTU HPC — TEMPORAL EVAL (greedy): Phase-1 ARM B (caption-last + temporal loss)
### Evaluates the loss-fix model (28633523, lambda=5 sigma=1) on the filtered
### FOR/LIVE/P501 temporal test sets in the ANCHOROFFSET <a><f> format. Reports
### the FULL panel: t-IoU + auto audio-blind baselines + unique intervals +
### response health (unique captions, caption BLEU, MOS MAE). Exit criteria:
### mean t-IoU > 0.36 (beats all audio-blind play); probe band ~0.5 is target.
###
...
#BSUB -q gpuh100
#BSUB -J eval-gc-timelast-softloss-temporal
#BSUB -n 4
...
#BSUB -M 48GB
#BSUB -W 4:00
#BSUB -o logs/eval_timelast_softloss_temporal_%J.out
#BSUB -e logs/eval_timelast_softloss_temporal_%J.err
...
MODEL_PATH="$PROJECT_DIR/models/sft_gc_timelast_softloss_h100"
OUTPUT_DIR="$EXPERIMENT_DIR/results/evaluation/temporal/gc_timelast_softloss_greedy"
