...
#BSUB -M 48GB
#BSUB -W 24:00
#BSUB -o logs/sft_gc_timelast_h100_%J.out
#BSUB -e logs/sft_gc_timelast_h100_%J.err
...
torchrun --nproc_per_node=1 scripts/train/supervised-finetune.py \
    ...
    --data-root data \
    --model-name "$EXPERIMENT_DIR/models/sft_gc_timelast_timeaudio_h100" \
    --bf16 \
    ...
    --install-time-tokens \
    --wandb-run-name "sft-gc-timelast-timeaudio-h100"
