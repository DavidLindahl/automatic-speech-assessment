...
notify_exit() {
  ...
  payload=$(cat <<JSON
{"job_id":"${LSB_JOBID:-unknown}","job_name":"${LSB_JOBNAME:-eval-dpo-hf-warmup-fix}","status":"${status}","queue":"gpua40","host":"$(hostname)","workflow":"${WORKFLOW}","todo_file":"${TODO_FILE}","run_file":"${RUN_FILE}","model_path":"${MODEL_PATH}","results_path":"${RESULTS_PATH}"}
JSON
)
