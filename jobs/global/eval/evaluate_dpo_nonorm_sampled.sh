...
### path, output dir, and job name change. Model is models/dpo_full_sft_paired_nonorm
### (run 28596884). Sampling T=0.7, top-p=0.9 (same as cycle-1 sampled
### eval) so the diversity comparison is apples-to-apples.
### Submit with: bsub < jobs/global/eval/evaluate_dpo_cycle2_sampled.sh
### ============================================================

#BSUB -q gpuh100
#BSUB -J eval-dpo-nonorm-sampled
#BSUB -n 4
...
#BSUB -M 32GB
#BSUB -W 4:00
#BSUB -o logs/eval_dpo_nonorm_sampled_%J.out
#BSUB -e logs/eval_dpo_nonorm_sampled_%J.err
...
uv run python scripts/eval/evaluate.py eval-mos \
    --model-path "$EXPERIMENT_DIR/models/dpo_full_sft_paired_nonorm" \
    --output-dir "$EXPERIMENT_DIR/results/evaluation/dpo/dpo_nonorm_eval_sampled" \
...
echo "=========================================="
echo "DPO nonorm sampled evaluation complete: $(date)"
echo "=========================================="
