...
### Submit with: bsub < jobs/global/eval/evaluate_dpo_delimiterfix_sampled.sh
###
...
uv run python scripts/eval/evaluate.py eval-mos \
    --model-path "$MODEL" \
    --output-dir "$EXPERIMENT_DIR/results/evaluation/dpo/dpo_paper_half_h100_delimiterfix_eval_sampled" \
...
uv run python scripts/diagnostics/dpo_sanity_check.py \
    "$EXPERIMENT_DIR/results/evaluation/dpo/dpo_paper_half_h100_delimiterfix_eval_sampled"
