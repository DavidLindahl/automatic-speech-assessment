...
def score_record(
    ...
    start_offset = pred.start - truth.start if pred is not None else None
    end_offset = pred.end - truth.end if pred is not None else None
    ...
    return {
        **row,
        ...
        "tiou": tiou,
        "offset_err": interval_offset_error(pred, truth) if pred is not None else None,
        "start_offset_err": start_offset,
        "end_offset_err": end_offset,
        "start_abs_err": abs(start_offset) if start_offset is not None else None,
        "end_abs_err": abs(end_offset) if end_offset is not None else None,
...
def write_results(
    ...
    start_offsets = [
        float(item.get("start_offset_err", item["pred_start"] - item["gt_start"]))
        for item in interval_parsed
    ]
    end_offsets = [
        float(item.get("end_offset_err", item["pred_end"] - item["gt_end"]))
        for item in interval_parsed
    ]
    offset_errors = [
        float(
            item.get(
                "offset_err",
                (
                    (item["pred_start"] - item["gt_start"])
                    + (item["pred_end"] - item["gt_end"])
                )
                / 2,
            )
        )
    ...
    metrics = {
        ...
        "expected_offset_error": mean(offset_errors),
        "mean_start_offset_err": mean(start_offsets),
        "mean_end_offset_err": mean(end_offsets),
