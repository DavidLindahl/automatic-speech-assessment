...
- **`build_temporal_inspector_site.py`**: Generates a web-based dashboard allowing developers to inspect predictions, listen to wav files, and check annotations.
- **`build_dpo_cycle_splices.py`**: Prepares preference pairs designed for assessing cycles of degradations.
- **`generate_dpo_temporal_factor.py`** & **`build_dpo_medium_shift.py`**: Generate specialized temporal alignment sets and shift-controlled comparison pairs.
