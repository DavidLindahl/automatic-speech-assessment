...
plt.rcParams.update({
    ...
    "axes.titlesize": 12.5, "figure.dpi": 150, "font.family": "DejaVu Sans",
})
...
ax.text(sizes[0],AUDIO_BLIND+0.02,"audio-blind floor (0.22)",color="#8a8a8a",fontsize=8.5)
...
for i,k in enumerate(sorted(losses,key=lambda k:int(k))):
    s=losses[k]
    if not s: continue
    ax.plot([p[0] for p in s],[p[1] for p in s],color=LINE5[i%len(LINE5)],lw=2,label=f"{int(k):,} clips")
...
# Fig 4.4: grouped bars, mIoU + MOS MSE side by side per size, dual axis
fig,ax1=plt.subplots(figsize=(7.6,4.6)); ax2=ax1.twinx()
x=list(range(len(sizes))); w=0.38
xl=[i-w/2 for i in x]; xr=[i+w/2 for i in x]
...
ax1.text(-0.40,AUDIO_BLIND+0.018,"audio-blind IoU floor (0.22)",color="#8a8a8a",fontsize=8,ha="left")
...
ax1.set_ylabel("Mean temporal IoU",color=SAGE); ax2.set_ylabel("MOS MSE",color=CLAY)
ax1.tick_params(axis="y",colors=SAGE); ax2.tick_params(axis="y",colors=CLAY)
ax1.set_ylim(0,1.0); ax2.set_ylim(0,max(mse)*1.6)
ax1.set_xticks(x); ax1.set_xticklabels(labels); ax1.set_xlabel("Training clips (1 per reference)")
...
h=[b1,b2]; ax1.legend(h,[x.get_label() for x in h],frameon=False,fontsize=9,loc="upper center",bbox_to_anchor=(0.5,-0.13),ncol=2)
