...
def test_alld_pipeline():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            batch["policy_input_ids"].shape[0] == expected_2n_batch_size
        ), f"Policy batch size should be {expected_2n_batch_size}"
        ...
        assert (
            batch["policy_labels"][0][0].item() == -100
        ), "Policy labels are not masking the prompt correctly!"
        ...
        assert (
            batch["ref_input_ids"].shape[0] == expected_2n_batch_size
        ), f"Reference batch size should be {expected_2n_batch_size}"
        assert "ref_labels" in batch, "Missing ref_labels"
        assert (
            batch["ref_labels"][0][0].item() == -100
        ), "Reference labels are not masking the prompt correctly!"
